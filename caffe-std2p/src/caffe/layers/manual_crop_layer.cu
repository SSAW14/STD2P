...
template <typename Dtype>
void PCropLayer<Dtype>::Forward_gpu(const vector<Blob<Dtype>*>& bottom,
  ...
  copy_kernel<<<CAFFE_GET_BLOCKS(lines), CAFFE_CUDA_NUM_THREADS>>>(
      ...
      bottom_data + bottom[0]->offset(0, 0, x_, y_), top_data);
}

template <typename Dtype>
void PCropLayer<Dtype>::Backward_gpu(const vector<Blob<Dtype>*>& top,
  ...
  if (propagate_down[0]) {
    ...
    copy_kernel<<<CAFFE_GET_BLOCKS(lines), CAFFE_CUDA_NUM_THREADS>>>(
        ...
        top[0]->height() * top[0]->width(), top[0]->width(),
        bottom[0]->height() * bottom[0]->width(), bottom[0]->width(),
        top_diff, bottom_diff + bottom[0]->offset(0, 0, x_, y_));
  }
}

INSTANTIATE_LAYER_GPU_FUNCS(PCropLayer);
