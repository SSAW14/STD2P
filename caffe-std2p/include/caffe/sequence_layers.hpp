 ...
 *        [1] style recurrent neural network (RNN). Implemented as a network
 *        unrolled the GRU computation in time.
 *
 *
 * The specific architecture used in this implementation is reproduced below:
 *     z_t := \sigmoid[ W_{hz} * h_{t-1} + W_{xz} * x_t + b_z ]
 *     r_t := \sigmoid[ W_{hr} * h_{t-1} + W_{xr} * x_t + b_r ]
 *     g_t := \tanh[ W_{hg} * (r_t .* h_{t-1}) + W_{xg} * x_t + b_g ]
 *     h_t := (1 - z_t) .* h_{t-1} + z_t .* g_t
 * In the implementation, the i, f, o, and g computations are performed as a
 * single inner product.
 *
 *
 * [1] Junyoung Chung, Caglar Gulcehre, KyungHyun Cho, Yoshua Bengio. "Empirical Evaluation of Gated Recurrent Neural Networks on Sequence Modeling."
 *     arxiv, 2014.
 *
 */
template <typename Dtype>
